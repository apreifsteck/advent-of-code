defmodule AdventOfCode.Solutions.Y2023.S7 do
use AdventOfCode.Solution
  ...
  def solve!(data) do
    data
    |> Stream.map(fn {l, r} -> MapSet.intersection(l, r) end)
  ...
  defp score(0), do: 0
  defp score(num_matches) when num_matches > 0, do: 2 ** (num_matches - 1) 


  def parse_data(data) do
    data
    |> Stream.flat_map(&Regex.split(~r/Card\s+\d+:/, &1, trim: true))
    |> Stream.map(&String.trim/1)
    |> Stream.map(&String.split(&1, "|"))
    |> Stream.map(fn [l, r] -> {l, r} end)
    |> Stream.map(&tuple_apply(&1, fn x -> String.split(x) end))
    |> Stream.map(&tuple_apply(&1, fn x -> Enum.map(x, fn inner_el -> String.to_integer(inner_el) end) end))
    |> Stream.map(&tuple_apply(&1, fn x -> MapSet.new(x) end))
    |> Enum.to_list()
    # |> dbg()
  end

  defp tuple_apply({l, r}, func) do
    {func.(l), func.(r)}
  end
